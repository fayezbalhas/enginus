      ...
      {steps.map((s, i) => (
        <div key={i} style={{ padding: '8px 10px', borderLeft: '2px solid #1e1e1e', marginBottom: '6px', background: '#0c0c0c', borderRadius: '0 4px 4px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
            {s.clause && (
              <span style={{ fontSize: '9px', fontWeight: 700, color: '#cc0000', background: 'rgba(204,0,0,0.08)', padding: '1px 6px', borderRadius: '3px', whiteSpace: 'nowrap' }}>
                {s.clause}
              </span>
            )}
...
interface ResultCardProps {
  title: string; mainLabel: string; mainValue: string
  secondLabel?: string; secondValue?: string
  utilization: number; pass: boolean; unit?: string
}
...
export function ResultCard({ title, mainLabel, mainValue, secondLabel, secondValue, utilization, pass, unit }: ResultCardProps) {
  ...
  return (
    <div style={{ background: '#111', border: `1px solid ${pass ? '#1e1e1e' : 'rgba(239,68,68,0.3)'}`, borderRadius: '8px', padding: '16px', transition: 'border-color 0.3s' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        ...
        <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 10px', borderRadius: '4px', background: pass ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)', color: pass ? '#22c55e' : '#ef4444', letterSpacing: '0.05em' }}>
          {pass ? 'PASS' : 'FAIL'}
        </span>
      </div>
      <div style={{ display: 'flex', gap: '16px', marginBottom: '10px' }}>
        <div>
          ...
          <div style={{ fontSize: '18px', fontWeight: 700, fontFamily: "'Space Grotesk', monospace", color: '#f0f0f0' }}>
            {mainValue}{unit && <span style={{ fontSize: '11px', color: '#555', marginLeft: '4px' }}>{unit}</span>}
          </div>
        </div>
        {secondLabel && (
          <div>
            ...
            <div style={{ fontSize: '18px', fontWeight: 700, fontFamily: "'Space Grotesk', monospace", color: pass ? '#22c55e' : '#ef4444' }}>
              {secondValue}{unit && <span style={{ fontSize: '11px', color: '#555', marginLeft: '4px' }}>{unit}</span>}
            </div>
          </div>
        )}
      </div>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
          ...
          <span style={{ fontSize: '10px', fontWeight: 700, color: util > 100 ? '#ef4444' : '#888', fontFamily: "'Space Grotesk', monospace" }}>{fmt(util, 1)}%</span>
        </div>
        <div style={{ background: '#1a1a1a', borderRadius: '3px', height: '5px', overflow: 'hidden' }}>
          <div style={{ width: `${barWidth}%`, height: '100%', borderRadius: '3px', background: util > 100 ? '#ef4444' : util > 85 ? '#f59e0b' : '#22c55e', transition: 'width 0.4s ease, background 0.4s ease' }} />
        </div>
      </div>
    </div>
  )
...
interface SummaryRow {
  check: string; required: string; provided: string; utilization: number; pass: boolean
}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #1a1a1a' }}>
              {['Check', 'Required', 'Provided', 'Util.', 'Status'].map(h => (
                <th key={h} style={{ textAlign: h === 'Status' ? 'center' : h === 'Check' ? 'left' : 'right', padding: '6px 8px', color: '#555', fontWeight: 600, fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #111' }}>
                ...
                <td style={{ padding: '7px 8px', textAlign: 'center' }}>
                  <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 8px', borderRadius: '3px', background: r.pass ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)', color: r.pass ? '#22c55e' : '#ef4444' }}>
                    {r.pass ? 'PASS' : 'FAIL'}
                  </span>
                </td>
              </tr>
