...
import { type SlabBarOption } from './calculations'
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #1a1a1a' }}>
              ...
              <th style={{ textAlign: 'center', padding: '6px 8px', color: '#555', fontWeight: 600, fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Dir.</th>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #111' }}>
                ...
                <td style={{ padding: '7px 8px', color: '#666', textAlign: 'center', fontFamily: "'Space Grotesk', monospace", fontSize: '11px' }}>{r.direction || '—'}</td>
...
export function SlabBarSchedulePanel({ items, As_req, direction }: { items: SlabBarOption[]; As_req: number; direction: string }) {
  if (items.length === 0) return null
  return (
    <div style={{ marginTop: '8px' }}>
      <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.06em', color: '#555', textTransform: 'uppercase', marginBottom: '6px' }}>
        {direction} (As,req = {fmt(As_req, 0)} mm²/m)
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
        {items.map((bs, i) => (
          <div key={i} style={{
            background: '#0d0d0d', border: `1px solid ${bs.sufficient ? '#1e1e1e' : 'rgba(239,68,68,0.2)'}`,
            borderRadius: '6px', padding: '6px 10px', textAlign: 'center', minWidth: '80px',
          }}>
            <div style={{ fontSize: '12px', fontWeight: 700, fontFamily: "'Space Grotesk', monospace", color: bs.sufficient ? '#f0f0f0' : '#666' }}>
              {bs.label}
            </div>
            <div style={{ fontSize: '9px', color: '#555', marginTop: '2px' }}>
              {fmt(bs.As_prov, 0)} mm²/m
              <span style={{ marginLeft: '3px', color: bs.sufficient ? '#22c55e' : '#ef4444' }}>
                {bs.sufficient ? '✓' : '✗'}
              </span>
            </div>
          </div>
